 ...
 *******************************************************************************/
 ...
 /******************************************************************************
  * Description : This functions opens the file with requested mode(Read/Write). 
  ...
  * Arguments   : char *: Name of the file to be opened.
                  uint8_t: READ, WRITE, APPEND
                  char *: pointer to update the file opening status.

  * Return      : fileConfig_st *: Pointer to file structure if opened successfully.
...
/******************************************************************************
 * Description : This functions closes the file and releases the memory used by the file.
                 Once the file is closed, no more file operation are allowed.

 * Arguments   : fileConfig_st *: structure pointer containing the file info that needs to be closed.
...
/******************************************************************************
 * Description : This functions reads a byte of data from the file.
 ...
 * Arguments   : fileConfig_st *: structure pointer containing the file info from which the data needs to be read.

 * Return      : char : Byte of data read from file.
 *****************************************************************************/
...
/******************************************************************************
 * Description : This functions writes a byte of data to the file.
                 It checks whether the requested file is opened or not, it writes the data only if the file is open.

 * Arguments   : fileConfig_st *: structure pointer containing the file info where the data needs to be written.
...
void FILE_PutCh (fileConfig_st *filePtr, char data)
{
    ...
    {
        if((filePtr->endOfFileDetected == 0) && (data != EOFF))
        {
...
void FILE_Putstr(fileConfig_st *filePtr,char str[]){
    int i;
    for(i=0;i<strlen(str);i++){
        FILE_PutCh(filePtr, str[i]);
    }
}
 ...
 ************************************************************************************************************************************************/
 /******************************************************************************
  * Description : extract_DataBySize

  * Arguments   : uint8_t *ptr,uint8_t size
...
/******************************************************************************
  * Description : getBootSectorData

  * Arguments   : none

  * Return      : SDCARD_INIT_SUCCESSFUL , SDCARD_FAT_INVALID
  *****************************************************************************/
...
/***************************************************************************
 Description : to calculate first sector address of any given cluster
...
/***************************************************************************
  Description : get cluster entry value from FAT to find out the next cluster in the chain
...
/********************************************************************************************
  Description : to get or set next free cluster or total free clusters in FSinfo sector of SD card
...
/***************************************************************************
   Description : to search for the next free cluster in the root directory
...
/***************************************************************************
  Description : to convert normal short file name into FAT format
...
uint16_t file_GetDate(void)
{
    uint8_t day = 2, month=6;
    uint16_t fat32Date,year=2021;
