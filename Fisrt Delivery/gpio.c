...
void GPIOF_Init()
{
	...
	while ((SYSCTL_PRGPIO_R & PORTF) == 0);           /*delay until the SYSCTL is enabled*/
...
/************************** Enable External interrupt on PF4 **************************/
    GPIO_PORTF_IS_R  &=~ SWITCH_1;                  /* falling edge or 0xff for rising edge*/
    GPIO_PORTF_IBE_R &=~ SWITCH_1;                  /*to low level or falling edge */
    GPIO_PORTF_IEV_R &=~ SWITCH_1;                  /*to low level or falling edge*/
    GPIO_PORTF_ICR_R |= SWITCH_1;                   /* pin_num*/
    GPIO_PORTF_IM_R  |= SWITCH_1;                   /* unmask interrupt */
/********************************** NVIC_IP_PRIORITY *********************************/
    NVIC_PRI7_R = (3<<5);                           /*set priority 3*/
    NVIC_EN0_R |=(1<<30);                           /*set Interrupt*/ 
}
/******************************************************************************
...
void GPIOF_Handler(void)
{
    ...
    g_Interrupt_Flag ^= 0x1;                        /*set Interrupt_Flag */

    delay_ms();                                   /* delay 1msec*/
}
